...
public class CardSelectScreen implements Screen {
    ...
    public static Texture drawCard(Card card) {
        Pixmap pixmap;
        if (card != null) {
            pixmap = new Pixmap(Gdx.files.internal("Card_" + card.getType() + ".png"));

            pixmap.drawPixmap(drawNumber(card.getPriority() / 100), 44, 14);
            pixmap.drawPixmap(drawNumber((card.getPriority() / 10) % 100), 52, 14);
            pixmap.drawPixmap(drawNumber((card.getPriority()) % 10), 60, 14);
        } else {
